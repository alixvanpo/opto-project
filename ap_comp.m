function [u,v,C1,O1,O2,C2] = ap_comp(iapp_stim, cond)
...
% ChR2 CURRENT
g_chr = cond;                                        % conductance of ChR (val 0.4-0.75)
exp_lvl = 1;                                         % expression level of ChR2, default is 1
gamma = 0.1;                                         % ratio O1/O2 (val 0.05-0.1)
G = @(v) (10.6408 - 14.6408*exp(-v/42.7671))/v;      % voltage rectification function
E_chr = 0;                                           % reversal potential of ChR (val -5 - +10)

ichr = @(v,O1,O2) exp_lvl*g_chr*(O1 + gamma*O2)*G(v)*(v-E_chr);     % ChR current
...
% rate constants
Gd1 = 0.084;                        % O1 to C1   (val 0.084-0.11) - paper min
Gd2 = 0.1254;                       % O2 to C2   (val 0.025-0.1254) - paper max
Gr = 0.004;                         % C2 to C1   (val 0.0004-0.0040) - paper max
e12 = 0.011;                         % O1 to O2   (val 0-0.03) - paper 0.011
e21 = 0.008;                        % O2 to O1   (val 0.008-0.015) - paper min
...
du = @(u,v,iapp,O1,O2) - k*u*(u-b)*(u-1) -u*v + iapp - Rm*ichr(v,O1,O2);       % fast
